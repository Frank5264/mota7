...
cha = "store_gon"
api_id = '25052437'
api_hash = '4d88ea14125a69234d4e8fe0cdc24b38'

client = TelegramClient('session', api_id, api_hash)
client.start()

# Send a message
start_c= "♾️ بدا فحص المتاحات ➿️"
message_id =348
client.edit_message(entity=cha, message=message_id, text=start_c)

frank = 'EEM_EM'

client.send_message(frank ,f"بدا الفحص     \n جار فحص المتاحات وسيتم ارسال المتاحات تلقائيا الي القناة الخاصه بك \n  معرف القناة  : @store_gon")
...
es = '51987448093:QZWYAAACbksrJZ:2:SAeygfvR77u38bag9uaOIfAzMvBA7tb1RZk3MNRWAP'
...
"username":email,"password":"paswgwgshhddv","device_id":uis,
 ...
 if "Please check your username and try again." in req:
...
''')
 	...
 	client.edit_message(entity=cha, message=message_id, text=tmm)
 	print(f'''  
 {F}! {F}- {F}Good     » 「{v}」
 {Z}! {Z}- {Z}BAD GM   » 「{b}」
 {X}! {X}- {X}BAD IG   » 「{n}」
 ...
 elif "The password you entered is incorrect." in req:
 			os.system('cls'if os.name=='net'else'clear')
 			v+=1
 			tmm=(f''' 
《●》Good     » 「{v}」
